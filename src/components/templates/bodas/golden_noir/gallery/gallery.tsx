...
export function PhotoGallerySection({ config }: PhotoGalleryProps) {
  ...
  // Reemplazo de la URL local por la de Cloudinary como fallback
  ...
  return (
    <section className="relative py-12 md:py-16 overflow-hidden text-white font-sans">
      ...
      <div className="container mx-auto px-6 mb-8 text-center relative z-10">
        <h3 className="text-3xl md:text-5xl font-serif italic text-white mb-2 drop-shadow-lg">
          Momentos Inolvidables
        </h3>
        <div className="w-12 h-px bg-[#b5a47a]/60 mx-auto" />
      </div>

      {/* Carrusel de Fotos Animado */}
      {fotos.length > 0 && (
          ...
          <div className="max-w-2xl mx-auto p-1.5 bg-white/10 rounded-xl border border-white/20">
            <div className="relative aspect-video overflow-hidden bg-black rounded-lg group border border-[#b5a47a]/30">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/40 transition-colors pointer-events-none">
                 <div className="w-16 h-16 flex items-center justify-center rounded-full bg-white/20 backdrop-blur-md border border-white/40 text-white">
      ...
      <AnimatePresence>
        {isVideoModalOpen && (
          ...
          >
            ...
            <motion.div 
              className="touch-auto w-full max-w-5xl flex items-center justify-center" 
              onClick={(e) => e.stopPropagation()}
            >
                <video 
                  src={videoUrl} 
