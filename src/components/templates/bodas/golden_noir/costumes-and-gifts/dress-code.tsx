...
export default function Details({ config }: WeddingDetailsProps) {
    ...
    <section className="relative py-16 md:py-24 overflow-hidden bg-transparent">
      ...
      <DetailModal isOpen={activeModal === "gift"} onClose={() => setActiveModal(null)} title="Regalos" icon={Landmark}>
        <div className="space-y-4">
          ...
          <CopyItem label="Titular" value={config.holderName || "Novios"} />
          <CopyItem label="CBU / CVU" value={config.cbu || "000000000000000000000"} />
          <CopyItem label="Alias" value={config.alias || "ALIAS.PENDIENTE"} />
          {config.bankName && (
