...
export function MusicSuggestion({ eventId }: MusicSuggestionProps) {
  ...
  return (
    <section className="relative py-16 md:py-24 overflow-hidden bg-[url('/images/img-grafitis/radio.png')] bg-cover bg-center mb-[-1px]">
      ...
      {/* MODAL RESPONSIVO CON BLOQUEO DE SCROLL */}
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md bg-white/40 backdrop-blur-2xl rounded-[2rem] p-6 md:p-10 shadow-2xl z-10 border border-white/40"
            >
              <button 
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                <div className="space-y-2">
                  ...
                  {[1, 2, 3].map((num) => (
                    <div key={num} className="bg-white/60 p-3 rounded-xl flex items-center gap-2 shadow-sm">
                      <span className="text-[#d29b7b] font-black text-xs">{num}.</span>
                      <input
                        type="text"
                        name={`tema${num}`}
                        ...
                        className="w-full bg-transparent border-none outline-none text-black font-bold text-xs placeholder:text-black/20 uppercase"
                      />
                    </div>
                  ))}
                </div>
