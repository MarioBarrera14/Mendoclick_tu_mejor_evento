...
export function Hero({ eventName, heroImage, eventDate }: HeroProps) {
  ...
  const currentImage = (heroImage && heroImage !== "") 
    ? heroImage 
    : "/img_boda/grafiteros.webp";

  // Fecha desde DB o fallback
  const finalDate = eventDate || "2026-09-12";

  const [timeLeft, setTimeLeft] = useState({ dias: 0, horas: 0, min: 0, seg: 0 });

  useEffect(() => {
    const targetDate = new Date(`${finalDate}T20:00:00`);
  ...
  // Formateo de fecha para el diseño original: DD . MM . YYYY
  const displayDate = finalDate.split("-").reverse().join(" . ");
