...
function LocationCard({ location, onOpen }: { location: Location; onOpen: () => void }) {
  return (
    <motion.div 
    ...
    >
      ...
      <div className="text-center px-2 relative z-10">
        ...
        <h3 className="text-lg md:text-xl font-serif italic text-white mb-1 tracking-tight uppercase drop-shadow-md transition-all duration-500 group-hover:drop-shadow-lg">
          {location.title}
        </h3>
        <div className="w-8 h-px bg-[#b5a47a]/40 mx-auto mb-2" />
        <p className="text-[#b5a47a] text-[9px] uppercase font-bold tracking-[0.2em] opacity-80 group-hover:opacity-100 transition-opacity drop-shadow-sm">
          Descubrir detalles
        </p>
      </div>
    </motion.div>
  ...
  const locations: Location[] = [
    {
      ...
      time: "20:00hs",
      placeName: "Parroquia Natividad del Señor",
      address: "Av. Eugenia Tapia de Cruz 524, Escobar",
    ...
    },
    {
      ...
      date: "12 de Septiembre, 2026",
      time: "21:30hs",
      placeName: "Estancia La Soñada del Pilar",
      address: "Sor Teresa y Cabo de Hornos, Pilar",
      googleMapsUrl: "https://maps.google.com"
    }
  ];
        ...
        {selectedLocation && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-lg bg-white rounded-[2rem] overflow-hidden shadow-[0_50px_100px_-20px_rgba(0,0,0,0.9)]"
            >
              <button 
                onClick={() => setSelectedLocation(null)}
                className="absolute top-6 right-6 z-50 p-2 text-gray-400 hover:text-black transition-all"
              >
                <X size={24} />
              </button>

              <div className="p-10 flex flex-col items-center text-center">
                <div className="text-[#b5a47a] mb-4 drop-shadow-sm">{selectedLocation.icon}</div>
                ...
                <div className="space-y-8 w-full text-gray-600">
                  ...
                  <div>
                    ...
                    <p className="text-xl text-gray-800 font-serif italic leading-tight mb-2">{selectedLocation.placeName}</p>
                    <p className="text-sm font-light text-gray-400 px-10">{selectedLocation.address}</p>
                  </div>
