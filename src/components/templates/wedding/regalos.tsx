...
// MODAL ACTUALIZADO CON TRANSPARENCIA
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            // CAMBIO: bg-white/30 y backdrop-blur-xl para el fondo del modal
    ...
    <section className="relative py-12 w-full flex items-center justify-center bg-[url('/images/img-grafitis/mural.jpg')] bg-cover bg-center px-4 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl flex flex-col md:flex-row gap-6 items-stretch justify-center font-['Permanent_Marker',_cursive]">
        ...
        >
          ...
          <p className="text-black text-xs md:text-sm font-medium mb-6 flex-grow font-sans">
            Para esta noche especial, el código de vestimenta es {dressCode || "Elegante Sport"}.
          </p>
      ...
      {/* MODALES CON DISEÑO TRANSPARENTE */}
      <DetailModal isOpen={activeModal === "dress"} onClose={() => setActiveModal(null)} title="Dress Code">
        <p className="text-black font-sans font-medium">{dressDescription || "Vestimenta formal/elegante."}</p>
      </DetailModal>

      <DetailModal isOpen={activeModal === "gift"} onClose={() => setActiveModal(null)} title="Datos Bancarios">
        <div className="space-y-3 font-sans">
          <div className="bg-white/40 p-3 rounded-xl mb-4 border border-white/20">
            <p className="text-[9px] uppercase text-black/60 font-black mb-1 text-center">Titular</p>
            <p className="text-black font-bold text-base leading-tight text-center">{holderName || "Mario & Ariadna"}</p>
          </div>
          <CopyButton label="CBU/CVU" text={cbu || "00000000000000000"} />
          <CopyButton label="Alias" text={alias || "ROCK.BODA.MARIO"} />
          {bankName && <p className="text-[10px] text-black/60 font-bold uppercase pt-2 text-center">{bankName}</p>}
        </div>
      </DetailModal>
       {/* SEPARADOR GRAFITERO SUPERIOR */}
