...
export function RSVP({ config }: RSVPProps) {
    ...
    try {
      const response = await fetch("/api/guests", {
        ...
        body: JSON.stringify({
          ...
          dietary: formData.dietary || "Ninguna",
          message: formData.message,
          confirmados: formData.attendance === "YES" ? formData.confirmados : 0,
        }),
      });
  ...
  return (
    <>
      <section className="relative py-20 md:py-24 overflow-hidden font-sans bg-[#b4a178] [clip-path:polygon(0_4%,100%_0,100%_98%,0_100%)] mt-[-40px] mb-[-40px]">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative w-full max-w-md bg-white p-8 rounded-sm shadow-2xl flex flex-col max-h-[95vh] text-gray-800" onClick={(e) => e.stopPropagation()}>
              ...
              {!isValidated ? (
                <div className="py-4 text-center font-sans">
                  ...
                  <input type="text" value={familyCode} onChange={(e) => setFamilyCode(e.target.value.toUpperCase())} placeholder="TU CÓDIGO" className="w-full bg-gray-50 border-b border-gray-200 py-4 text-center text-2xl font-light tracking-[0.3em] outline-none text-gray-700 uppercase" />
                      ...
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                        ...
                        <div className="space-y-2 text-center">
                          ...
                          <div className="flex items-center justify-center gap-6">
                            <button onClick={() => setFormData(p => ({...p, confirmados: Math.max(1, p.confirmados - 1)}))} className="text-[#b4a178] p-1 border border-gray-200 rounded-full hover:bg-gray-50 transition-colors"><Minus size={16}/></button>
                            <span className="font-light text-xl text-gray-700 w-8">{formData.confirmados}</span>
