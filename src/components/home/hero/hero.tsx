...
export const Hero = () => {
  ...
  const clipPathClass = "[clip-path:polygon(0_0,100%_0,100%_92%,0%_100%)]"; 
  ...
  return (
    ...
    <section className="relative min-h-[100svh] flex items-center justify-center overflow-hidden bg-[#f9f9f9] z-10">
      ...
      <motion.div 
        ...
        // mt-0 y gap-1 en mobile para que no se estire el contenido verticalmente
        className="container mx-auto z-20 px-6 mt-0 flex flex-col lg:flex-row items-center justify-center lg:justify-between gap-1 md:gap-12 relative h-full"
      >
        {/* --- SMARTPHONE --- */}
        {/* Achicamos la escala en mobile (0.5) para que no ocupe toda la pantalla */}
        ...
        <div className="flex-1 text-center lg:text-left order-2 lg:order-1 w-full max-w-xl relative z-30 -mt-8 md:mt-0">
          ...
          >
            {/* Ajustamos tamaños de fuente para que no rompa en pantallas chicas */}
            ...
            <p className={`max-w-xs mx-auto lg:mx-0 text-white text-[8px] xs:text-[10px] sm:text-xs md:text-sm uppercase tracking-[0.2em] leading-tight mb-6 font-bold ${textShadowClass}`}>
              Tené hoy tu invitación <br className="hidden sm:block" />
              <span className="text-[#33aba1] italic">interactiva y original</span>
            </p>

            {/* Gap reducido y botones un poco más compactos para mobile */}
